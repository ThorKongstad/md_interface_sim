...
def main(structure: str, mode: str, xc: str, tb_method:str, temperature: float, brendsen_tau: float, time_step: float, kpts: tuple[int, int, int] = (1, 1, 1), static_pwd: bool = False):
    atoms = read(structure)

    sti = os.path.dirname(os.path.realpath(structure)) if not static_pwd else './'
    name = os.path.basename(structure).split('.')[0]

    tb_calc_par_dict = dict(
        method=tb_method,
    )
    ...
    tb_calc_pickle = str(pickle.dumps(tb_calc_par_dict))

    with db.connect(f'{sti}/{name}-{tb_method}.db') as db_obj:
        db_obj.write(atoms=atoms, kinitic_E=0, temperature=temperature, brendsen_tau=brendsen_tau, time=0, time_step_size=time_step, data=dict(xc_calc_pickle=xc_calc_pickle, tb_calc_pickle=tb_calc_pickle))
...
if __name__ == '__main__':
    ...
    parser.add_argument('tb_method', type=str)
