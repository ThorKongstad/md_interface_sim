...
def db_mace_observer(atoms: Atoms, database_dir: str, temperature: float, brendsen_tau: float, time_step_size: float, xc_calc_par_pickle: bytes, mace_calc_par_pickle: Optional[bytes] = None, current_time: Optional[float] = None):
    ...
    db_obj.write(atoms=atoms, temperature=temperature, brendsen_tau=brendsen_tau, time=cur_time, time_step_size=time_step_size, data=dict(mace_calc_pickle=(str(mace_calc_par_pickle) if mace_calc_par_pickle else str(pickle.dumps(atoms.calc.parameters))), xc_calc_pickle=str(xc_calc_par_pickle)))
...
def start_dft(submission_script: str, dft_script: str, db_mace: str):
    if world.rank == 0:
        current_time = globals().get('cur_time')
        call([submission_script, dft_script, db_mace, str(current_time)])
...
def main(md_db: str, n_steps: int, run_until: bool = False, dft_interval: int = False, submission_script: str = '/lustre/hpc/kemi/thorkong/katla_submission/submit_katla_GP241_static'):
    ...
    with db.connect(md_db) as db_obj:
        ...
        atoms.set_calculator(MACECalculator(**pickle.loads(mace_calc_pickle)))
 ...
 #       free_atoms = len(atoms) - sum([len(con.index) for con in atoms.constraints]) # this will only work in newer version of ase and older fixatoms in older version, since indicies are called a in some places
    ...
    dyn = Langevin(atoms=atoms, timestep=time_step * units.fs, temperature_K=temperature, friction=0.01 / units.fs, rng=random)
    dyn.attach(db_mace_observer,
               ...
               mace_calc_par_pickle=mace_calc_pickle,
               )
