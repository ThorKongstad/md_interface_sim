...
def main(md_db: str, cur_time: int, out_db: Optional[str] = None):
    ...
    with db.connect(out_db) as db_obj:
        db_obj.write(atoms=atoms, kinitic_E=atoms.get_kinetic_energy(), fermi_E=fermi_E, work_top=work_function_top, work_bot=work_function_bot, temperature=temperature, brendsen_tau=brendsen_tau, time=cur_time, data=dict(xc_calc_pickle=str(xc_calc_pickle), mace_calc_pickle=str(mace_calc_pickle)))
