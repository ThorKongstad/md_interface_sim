...
def main(structure: str, mode: str, xc: str, mace_model:str, temperature: float, brendsen_tau: float, time_step: float, kpts: tuple[int, int, int] = (1, 1, 1), static_pwd: bool = False):
    atoms = read(structure)

    sti = os.path.dirname(os.path.realpath(structure)) if not static_pwd else './'
    name = os.path.basename(structure).split('.')[0]

    mace_calc_par_dict = dict(
        model_path=mace_model,
        device='cuda'
    )

    kpts_dict = dict(kpts=kpts) if mode == 'pw' else dict()

    xc_calc_par_dict = dict(
        mode=mode,
        basis='dzp',
        #setups={'Pt': '10'},
        xc=xc,
    ...
    xc_calc_pickle = str(pickle.dumps(xc_calc_par_dict))
    mace_calc_pickle = str(pickle.dumps(mace_calc_par_dict))
    mace_model_name = os.path.basename(mace_model).split('.')[0]

    with db.connect(f'{sti}/{name}-{mace_model_name}.db') as db_obj:
        db_obj.write(atoms=atoms, kinitic_E=0, temperature=temperature, brendsen_tau=brendsen_tau, time=0, time_step_size=time_step, data=dict(xc_calc_pickle=xc_calc_pickle, mace_calc_pickle=mace_calc_pickle))
...
if __name__ == '__main__':
    ...
    parser.add_argument('mace_model', type=str)
    ...
    main(structure=args.traj_struc, mode=args.mode, xc=args.XC, mace_model=args.mace_model, temperature=args.temperature,
         brendsen_tau=args.brendsen_tau, time_step=args.time_step_size, kpts=args.kpts, static_pwd=args.static)
