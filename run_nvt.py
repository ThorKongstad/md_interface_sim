...
def db_observer(atoms: Atoms, database_dir: str, temperature: float, time_step_size: float, calc_par_pickle: Optional[bytes] = None,):
    ...
    work_function_top = mean_elec_pot_z[-1 if not (atoms.calc.parameters.get('mode') if isinstance(atoms.calc.parameters.get('mode'), str) else atoms.calc.parameters.get('mode').get('name')).lower() == 'pw' else -3] - fermi_E
    work_function_bot = mean_elec_pot_z[0 if not (atoms.calc.parameters.get('mode') if isinstance(atoms.calc.parameters.get('mode'), str) else atoms.calc.parameters.get('mode').get('name')).lower() == 'pw' else 2] - fermi_E
